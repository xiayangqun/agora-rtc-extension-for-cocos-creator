 ...
 *
 * Usage:
 *   node scripts/generate-nativevalue-to-se.js
...
const OUTPUT_H = path.join(ROOT, "native", "bindings", "manual", "RtcNativeValueToSe.h");
const OUTPUT_CPP = path.join(ROOT, "native", "bindings", "manual", "RtcNativeValueToSe.cpp");
...
function parseExistingImpls(middleContent) {
    ...
    const protectedBodies = []; // full text of protected function implementations
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (prev === DO_NOT_GEN) {
            ...
            if (m) {
                ...
                // Extract the full function body: from the "// do not gen" comment
                ...
                protectedBodies.push(chunk.join("\n"));
            }
        }
    }
    return { protectedStructs, protectedBodies };
}
...
function main() {
    ...
    const { protectedStructs, protectedBodies } = parseExistingImpls(cppParts.middle);
    ...
    const exampleComment = [
        `// ─────────────────────────────────────────────────────────────────────────────`,
        `// This file is AUTO-GENERATED by scripts/generate-nativevalue-to-se.js.`,
        `//`,
    ...
    // Write .h — preserve protected declarations from existing file
    const existingDeclLines = hParts.middle.split("\n");
    const protectedDecls = [];
    for (let i = 1; i < existingDeclLines.length; i++) {
        const prev = existingDeclLines[i - 1].trim();
        const curr = existingDeclLines[i].trim();
        if (prev === DO_NOT_GEN && curr.startsWith("bool sevalue_to_native(")) {
            protectedDecls.push(existingDeclLines[i - 1]); // the comment
            protectedDecls.push(existingDeclLines[i]);     // the declaration
        }
    }
    const protectedDeclSection = protectedDecls.length > 0
        ? "\n" + protectedDecls.join("\n") + "\n"
        : "";
    const newH =
        hParts.before + "\n" + exampleComment + newDecls.join("\n") + protectedDeclSection +
        USER_BLOCK_START + "\n" + userBlockH +
        USER_BLOCK_END + "\n" + hParts.after;
    fs.writeFileSync(OUTPUT_H, newH, "utf8");

    // Write .cpp — append preserved protected implementations after generated ones
    const protectedSection = protectedBodies.length > 0
        ? "\n" + protectedBodies.join("\n\n") + "\n"
        : "";
    const newCpp =
